...
if (function(jQuery) {
	...
	*/
	...
	function getServerAddress() {
        if (getShopifyDomain() == "crazy-apps-house.myshopify.com") {
            return "https://sticky-buy-now-button-cnorton.c9users.io";
        } else if (getShopifyDomain() == "preorder-tagging.myshopify.com"){
	...
	*/
	function setupEnv(shopifyProduct, shopSettings){
		...
		jQuery("head").append('<link rel="stylesheet" href="' + getServerAddress() + '/assets/sticky-buy-now-button.css?' + Math.random()*10 + '" type="text/css" />');
    ...
    shopURL:window.location.hostname,
		...
		hideBarAfterClick: shopSettings["hide_bar_after_click"],
      ...
      buyNowButtonText:shopSettings["buy_button_text"],
		...
		if(jQuery(fixedVars.buyButtonSelector).length > 0){
  ...
  function getCurrentVariantTitle(fixedVars){
	...
	function createVariantSelect(fixedVars){
		...
		if(variantFields.find("[type=radio]").length > 0){
			...
			jQuery(fixedVars.variantSelector).each(function(arrayIndex, item){
				...
				}else if(jQuery(item).find("input").length > 0 && jQuery(jQuery(item).find("input")[0]).attr("name")){
					fieldAttributes += " name='" + jQuery(jQuery(item).find("input")[0]).attr("name") + "' ";
				}
	...
	function attachVariantEventListeners(fixedVars){
		...
		if(jQuery(fixedVars.variantSelector).find("[type=radio]").length > 0){
          ...
          jQuery("body").on("change", ".radio-wrapper fieldset input,  .product__available-sizes:first input, .product__available-colors:first input, .swatch input, .swatch-element input, .radio-wrapper input", function() {
          ...
          jQuery(".fixedBuyBarVariants select").change(function() {
              ...
              if(jQuery(this).attr("id") != null){
	              ...
	              jQuery("form select[data-product-option='" + jQuery(this).attr("data-product-option") + "']").val(jQuery(this).val()); 
	              jQuery("form select[data-product-option='" + jQuery(this).attr("data-product-option") + "']").change();
              }
              
              var selectedValue = jQuery(this).val();
              
              if(jQuery("div[data-value='" + selectedValue + "'].panda-swatch").length > 0){
              	jQuery("div[data-value='" + selectedValue + "'].panda-swatch").trigger("click");
              }
          ...
          jQuery("body").on("change", fixedVars.variantSelector + ", " + fixedVars.variantSelector + " input, " + fixedVars.variantSelector + " select", function() {
              ...
              if(jQuery(this).attr("id") != null){
              	...
              	jQuery(".fixedBuyBarVariants select[data-product-option='" + jQuery(this).attr("data-product-option") + "']").val(jQuery(this).val());
              }
	...
	function formatPrice(currencySymbol, price){
		if(currencySymbol.indexOf("{{amount_no_decimals}}") !== -1){
			...
			return currencySymbol.replace("{{amount_no_decimals_with_comma_separator}}",String(Math.round(price)).replace(".",","));
			...
			return currencySymbol.replace("{{amount_no_decimals_with_space_separator}}",String(Math.round(price)).replace(","," "));
	...
	function updatePriceInBar(fixedVars){
		...
		for (var i = 0, len = fixedVars.productVariants.length; i < len; i++) {
			if(fixedVars.productVariants[i].id == getCurrentVariantId(fixedVars)){
				...
				productTitle = fixedVars.productTitle + " - " + fixedVars.productVariants[i].title;
				if(fixedVars.productVariants[i].compare_at_price == null || fixedVars.productVariants[i].compare_at_price == "" || parseInt(fixedVars.productVariants[i].price) >= parseInt(fixedVars.productVariants[i].compare_at_price)){
					originalPriceCode = "";
				}else{
  ...
  function updateCheckoutURLButton(fixedVars){
    ...
    // Usage!
	    ...
	    if(jQuery(fixedVars.buyButtonSelector).prop("disabled")){
	    	jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").text(fixedVars.soldOutText);
	    	jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").val(fixedVars.soldOutText);
	    }else{
	    	jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").text(fixedVars.buyButtonText);
	    	jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").val(fixedVars.buyButtonText);	    	
	    }
	...
	*/	
	function createBar(fixedVars){
		...
		var buyButtonCode = jQuery(fixedVars.buyButtonSelector).clone().text(buyButtonText),
	    ...
	    if(fixedVars.desktopSettings.paymentOptions && !fixedVars.onMobile){
	  	...
	  	jQuery(".fixedBuyBarButton button, .fixedBuyBarButton input, " + fixedVars.buyButtonSelector).click(function(){
	  	...
	  	jQuery(fixedVars.buyButtonSelector + ", .fixedBuyBarButton button, .fixedBuyBarButton input").click(function(){
	  	...
	  	jQuery(".fixedBuyBarButton button, .fixedBuyBarButton input").click(function(){
  			...
  			jQuery(".fixedBuyBarButton button, .fixedBuyBarButton input").text(fixedVars.addedToCartText);
  			jQuery(".fixedBuyBarButton button, .fixedBuyBarButton input").val(fixedVars.addedToCartText);
  			jQuery(".fixedBuyBarButton button, .fixedBuyBarButton input").prop("disabled", true);
  			...
  			}else if(getShopifyDomain() == "nutracelle.myshopify.com" || getShopifyDomain() == "frank-wilder.myshopify.com" || getShopifyDomain() == "stonewall-sheets.myshopify.com" || getShopifyDomain() == "rite-deals-store.myshopify.com" || getShopifyDomain() == "tresorethnique.myshopify.com" || getShopifyDomain() == "dansawesomestore.myshopify.com" || getShopifyDomain() == "edgy-deals-shop.myshopify.com" || getShopifyDomain() == "adams-warehouse.myshopify.com" || getShopifyDomain() == "sweepi.myshopify.com" || getShopifyDomain() == "hi-tempo.myshopify.com" || getShopifyDomain() == "gagons.myshopify.com" || getShopifyDomain() == "foreverglammed.myshopify.com" || getShopifyDomain() == "bibset.myshopify.com" || getShopifyDomain() == "payak.myshopify.com" || getShopifyDomain() == "gymbroclub.myshopify.com" || getShopifyDomain() == "phawa.myshopify.com" || getShopifyDomain() == "my-lavish-corner.myshopify.com" || getShopifyDomain() == "mpndesigns.myshopify.com"){
		...
		if(onMobile() && variantCode.length == 0 && fixedVars.mobileSettings.mobileDesign == "full"){
			jQuery("#fixedBuyBar.full").outerHeight(jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").outerHeight() + 10);
		...
		if(onMobile() && fixedVars.mobileSettings.mobileDesign == "full"){
			jQuery(".full .fixedBuyBarOriginalPrice, .full .fixedBuyBarPrice").css("line-height",(jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button").outerHeight() + 5) + "px")
		}
	}
	...
	function sizeTheQuantitySelector(variantCode){
		if(variantCode.length){
			if(!onMobile()){
				var quantitySelectorHeight = jQuery(".fixedBuyBarVariants select").outerHeight();
			}else{
				var quantitySelectorHeight = 48
			}
		}else{
			var quantitySelectorHeight = jQuery(".fixedBuyBarButton input, .fixedBuyBarButton button, fixedBuyBarButton a").outerHeight();
		}
		
		jQuery("#fixedBuyBar input[name=quantity]").outerHeight(quantitySelectorHeight);
	...
	function getTopOffset(fixedVars){
		...
		var fixedElements = jQuery("*").filter(function(){
			if(jQuery(this).css("position") === "fixed" && jQuery(this).css("top") === "0px" && jQuery(this).is(":visible") && jQuery(this).width() / jQuery("body").width() === 1 && jQuery(this).attr("id") != "fixedBuyBar" && jQuery(this).css("visibility") == "visible" && jQuery(this).css("opacity") > 0 && jQuery(window).height() != jQuery(this).height()){
				return this;
			}
		});
				        ...
				        jQuery(document).ready(function(){
				          if(fixedVars.onMobile){
				            jQuery.getScript(stickyJSCDN).done(function() {
				            	if(fixedVars.mobileSettings.barPosition=="top"){
												jQuery("#fixedBuyBar.mobile, #fixedBuyBar.full").sticky({topSpacing:0,getWidthFrom:"body",responsiveWidth:true});
											}else{
												jQuery("#fixedBuyBar.mobile, #fixedBuyBar.full").sticky({bottomSpacing:0,getWidthFrom:"body",responsiveWidth:true});
											}
				            });
				          }else{
				            jQuery.getScript(stickyJSCDN).done(function() {
